...
def get_textures(modelName):
    if(os.path.exists(modelName+'/textures_order.json')):
        ...
        dir_list = [[dir_list[0][x1], dir_list[1][x2], dir_list[2][x2], dir_list[3][x3]] 
                    for x1 in range(len(dir_list[0])) 
                        for x2 in range(min(len(dir_list[2]),len(dir_list[1]))) #增加了len(dir_list[2])
                            for x3 in range(len(dir_list[3]))]
